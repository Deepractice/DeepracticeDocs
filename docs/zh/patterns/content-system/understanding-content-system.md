...
PSO 的 Scope 定义有两种方式，背后是一个深刻的技术原理：**状态性（Statefulness）**。

**无状态（Stateless）内容**使用平行式 includes/excludes：
- 各部分相互独立，像乐高积木
- 可以任意组合，不影响系统状态
- 类似函数式编程中的纯函数

**有状态（Stateful）内容**使用递进式 progression：
- 每步改变系统状态，为下一步创建条件
- 必须按顺序执行，像盖房子必须先有地基
- 类似管道操作，有累积效应

这个原则极其重要，因为它决定了文档的组织方式。配置类文档通常是有状态的（每步都改变项目配置），而知识类文档通常是无状态的（阅读不改变任何状态）。理解了状态性，就能准确选择 Scope 的定义方式。
